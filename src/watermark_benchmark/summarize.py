...
def run(config_file, generations=None, include_paraphrase=False):
    # Load config
    config = (
    ...
    if not config.load_from_save:
        ...
        for g in generations:
            # print(f"g:{g}\ng attack:{g.attack}\ng efficiency:{g.efficiency}\ng watermark:{g.watermark}\n-------------")
            if not g.efficiency:
    ...
    if len(attack_list) > 1:
        if config.validation:
            ...
            robustness_summary = summarize_robustness(
                summary_attack, threshold=config.threshold, include_paraphrase=include_paraphrase,
            )
