...
def run(config_file, watermarks=None, save_full=False, fixed_keys=None):
    ...
    if config.inputs == 'debug' or config.inputs == 'test':
        # Randomly select k elements
        len_prompts = len(raw_prompts)
        k = 20
        print(f"Number of raw prompts: {len_prompts}, randomly choose {k} for testing...")
        raw_prompts = random.sample(raw_prompts, k)
        if config.inputs == 'debug':
            print(f"Only choose the 10th prompt for debugging: {raw_prompts[0]}")
            raw_prompts = raw_prompts[9:11]
    elif config.inputs == 'test10':
        # Randomly select k elements
        len_prompts = len(raw_prompts)
        k = 10
        print(f"Number of raw prompts: {len_prompts}, randomly choose {k} for testing...")
        ...
        len_prompts = len(raw_prompts)
        k=len(raw_prompts)
        print(f"Number of raw prompts: {len_prompts}, randomly choose {k} for full evaluation...")
        random.seed(666)
        # Shuffle the list
    ...
    for idx, device in enumerate(config.get_devices()):
        print("config devices: ", config.get_devices(), "current device: ", device)
